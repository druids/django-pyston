...
from germanium.tools.trivials import assert_equal, assert_in, assert_not_equal
...
class DataProcessorsTestCase(PystonTestCase):
    ...
    def get_file_url_response(self, data, call_url=None, serve_url='http://foo.bar/testfile.pdf'):
        data['contract'] = {
            'filename': 'testfile.pdf',
            'url': serve_url if not call_url else call_url,
        }
        with responses.RequestsMock(assert_all_requests_are_fired=False) as rsps:
            with open(os.path.join(settings.PROJECT_DIR, 'data', 'tests', 'pdf-sample.pdf'), 'rb') as f:
                content = f.read()
            self.serve_file(rsps, serve_url, content)
    ...
    def test_should_raise_bad_request_if_url_is_not_valid(self, number, data):
        ...
        resp = self.get_file_url_response(data, serve_url='hs://foo.bar/testfile.pdf')
        assert_http_bad_request(resp)
        errors = self.deserialize(resp).get('messages', {}).get('errors')
        assert_in('contract', errors)
    ...
    def test_should_raise_bad_request_if_file_is_unreachable(self, number, data):
        ...
        resp = self.get_file_url_response(data, call_url='http://foo.wbar/ttestfile.pdf')
