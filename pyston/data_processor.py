...
from pyston.utils.compatibility import get_model_from_rel_obj, get_related_from_descriptior, is_single_related_descriptor, is_multiple_related_descriptor
...
class ReverseMultipleDataPreprocessor(MultipleDataProcessorMixin, ResourceProcessorMixin, DataProcessor):
    ...
    def _create_or_update_reverse_related_objects_remove(self, data, key, data_item, rel_object):
        if isinstance(data, (tuple, list)):
            try:
                self._delete_reverse_objects(data, get_model_from_rel_obj(rel_object))
            except DataInvalidException as ex:
                self._append_errors(key, 'remove', ex.errors)
        else:
            self._append_errors(key, 'remove', self.INVALID_COLLECTION_EXCEPTION)
    ...
    def _create_or_update_reverse_related_objects_add(self, data, key, data_item, rel_object):
        if isinstance(data, (tuple, list)):
            try:
                self._create_and_return_new_object_pk_list(data, get_model_from_rel_obj(rel_object), self.inst,
    ...
    def _create_or_update_reverse_related_objects(self, data, key, data_item, model_descriptor):
        ...
        resource = self._get_resource(get_model_from_rel_obj(rel_object))
        if resource:
            ...
            try:
                del self.inst._prefetched_objects_cache[
                    self.get_related_from_descriptior(model_descriptor).field.related_query_name()
                ]
            except (AttributeError, KeyError):
                pass

    def _process_field(self, data, files, key, data_item):
        if (is_multiple_related_descriptor(self.model, key) and
            ((isinstance(data_item, dict) and set(data_item.keys()).union({'set', 'add', 'remove'})) or
             (isinstance(data_item, list)))):
            self._create_or_update_reverse_related_objects(data, key, data_item, getattr(self.model, key, None))
...
class ReverseDataPostprocessor(ResourceProcessorMixin, DataProcessor):
    ...
    def _process_field(self, data, files, key, data_item):
        if is_single_related_descriptor(self.model, key):
            self._create_or_update_single_reverse_related_objects(data, key, data_item, getattr(self.model, key, None))
